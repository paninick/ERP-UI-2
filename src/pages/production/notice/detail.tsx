...
export default function NoticeDetailPage() {
  ...
  const addColorPanel = () => {
    setDraft((prev) => prev ? {
      ...
      colorPanels: [
        ...prev.colorPanels,
        {id: createRowId('notice-color-panel'), title: '主色 - 新色', lines: [{id: createRowId('notice-color-line'), partName: '', colorNo: '', colorName: ''}]},
      ],
    } : prev);
  };
  ...
  const updateColorLine = (panelId: string, lineId: string, field: 'partName' | 'colorNo' | 'colorName', value: string) => {
    setDraft((prev) => prev ? {
      ...prev,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-2xl font-bold text-slate-900">打样通知详情</h2>
            <p className="text-sm text-slate-500">恢复表单信息、样衣信息、颜色组、物料明细、流转附件容器。</p>
          </div>
        </div>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm">
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[200px_1fr]">
          <div>
            <label className="mb-1 block text-sm text-slate-500">款式图片</label>
            <div className="flex h-40 items-center justify-center rounded-2xl border border-dashed border-slate-300 bg-slate-50 text-sm text-slate-400">
              {draft.baseInfo.styleImageName || '款式图片占位'}
            </div>
            <label className="mt-4 mb-1 block text-sm text-slate-500">附件列表</label>
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm text-slate-500">
              {draft.baseInfo.attachmentNames || '附件容器已保留，可接打印图纸、批注件、pdf 附件'}
            </div>
          </div>
          <div>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">样衣信息</h3>
            <p className="text-sm text-slate-500">恢复打样通知里的颜色 / 尺码 / 针型 / 数量分录，避免只剩主表。</p>
          </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {draft.sampleRows.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-3 py-8 text-center text-slate-400">暂无样衣信息</td>
                </tr>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">打样通知中的颜色组恢复，后续可以继续向打样工艺和 BOM 传递。</p>
          </div>
      ...
      <section className="rounded-3xl bg-white p-6 shadow-sm">
        <div className="mb-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-slate-500">主料、辅料分区恢复，保留打样通知和样衣 BOM、打样工艺之间的数据桥。</p>
          </div>
          <div className="flex gap-2">
            <button type="button" onClick={() => addMaterialRow('main')} className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">+ 厂供主料</button>
            <button type="button" onClick={() => addMaterialRow('auxiliary')} className="rounded-xl border border-slate-200 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50">+ 客供辅料</button>
          </div>
        </div>
          ...
          <div key={title} className="mb-6 last:mb-0">
            ...
            <div className="overflow-x-auto rounded-2xl border border-slate-200">
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 text-left text-slate-500">
                    {['供货方式', '产品编号', '产品名称', '成分', '主色', '部位', '色番', '色名', '采购数量', '备注'].map((header) => (
                      <th key={header} className="px-3 py-3">{header}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {(rows as NoticeMaterialRow[]).length === 0 ? (
                    <tr>
                      <td colSpan={10} className="px-3 py-8 text-center text-slate-400">暂无数据</td>
                    </tr>
                  ) : (rows as NoticeMaterialRow[]).map((row) => (
                    <tr key={row.id} className="border-t border-slate-100">
